...
st.write('---')
st.write("""
   To create this app, we had to curate data from prominent and popular media publications.
   We scraped available APIs and created dataframes based on the prominence of topics such as murder and crime.
   We then used this data to create a web application with visuals that represent different demographic information in regards to topics that show up frequently in the news. 
   There are interactive widgets that allow a reader to be able to get different interpretations of the data, along with time periods to narrow down the search. 
   This allows the data presented to be digestible, and specific to a reader’s needs.      
...
There is not a strong correlation between the rate at which events occur in the real world, and the rate at which articles about those events are published.

Ho: The correlation between events and their report-rate is strong.

Ha: The correlation between events and their report-rate is not strong.
...
1. Collected data from (INSERT SOURCES HERE)
...
with st.container():
    ...
    with col1:
        ...
        st.write(""" 
        With 8,857,300 rows of data, we have enough statistical power to show that on the topic of murder, the rate at which the New York Times publishes articles about said topic, has a moderate correlation with the rate at which events occur over time, with a correlation coefficient (Pearson’s) of 0.63.
        """)
    with col2:
        st.write("Heatmap explained: ")
        st.write('''
...
st.header("Result: ")
st.write("""The results we got from comparing our publication data to the real word values have led us to believe that news publications typically go for shock value over substance.
 When compared to data that comes from real life situations, we have observed that a lot of publications go for the more shocking and horrifying occurrences, the frequency of which may not an accurate foil to real life situations.
 """)
